...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      trigger: { cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", border: "none", borderRadius: "6px", background: "transparent", color: "inherit", padding: "4px 8px", fontSize: "12px", lineHeight: 1.4, whiteSpace: "nowrap" },
      triggerHover: { background: "var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,0.16))" },
      ...
      badgeEmpty: { flex: "0 0 auto", fontSize: "10px", lineHeight: "14px", padding: "0 5px", borderRadius: "4px", border: "1px solid rgba(128,128,128,0.45)", opacity: 0.7, whiteSpace: "nowrap" },
      badgeLive: { flex: "0 0 auto", fontSize: "10px", lineHeight: "14px", padding: "0 5px", borderRadius: "4px", border: "1px solid rgba(230,170,80,0.6)", color: "#e0b070", whiteSpace: "nowrap" },
    ...
    function Panel(props) {
      ...
      const headStyle = embedded ? Object.assign({}, S.head, { cursor: "default" }) : S.head;
      const head = h("div", { style: headStyle, onMouseDown: embedded ? undefined : startDrag, title: embedded ? undefined : "按住可拖动面板" },
        h("strong", null, "会话仓库"),
        h("div", { style: { display: "flex", gap: "6px", alignItems: "center" }, onMouseDown: (e) => e.stopPropagation() },
          ...
          embedded ? null : h("button", { type: "button", style: S.btn, onClick: onClose }, "关闭")
        )
      );
      ...
      const statLine = stats ? h("div", { style: S.stat },
        ...
        stats.liveCount ? h("span", { title: "这些会话仍被 dsh 加载在内存中；重启 dsh web 之前无法真正归档干净" }, "内存中 " + stats.liveCount + " 个") : null,
      ...
      const rows = list.length === 0
        ...
        : list.map((it) => {
            ...
            // 「内存中」= dsh 进程仍加载着它（持有 session.lock）。这类会话只按文件 mtime 看是「闲置」的，
            // 但归档后 dsh 的会话列表照旧显示它 —— 正是「归档没生效」的成因，所以单独标出来。
            if (tab === "current" && it.live) titleChildren.push(h("span", { key: "lv", style: S.badgeLive, title: "该会话已被 dsh 加载在内存中（进程持有 session.lock）。移走文件不会让它在会话列表里消失，需先重启 dsh web 才能归档干净。" }, "内存中"));
            if (tab === "archived" && it.auto) titleChildren.push(h("span", { key: "a", style: S.badgeAuto, title: "由自动维护归档" }, "自动"));
            // 区分「标题丢了」和「本来就没有标题」：后者是打开后未产生对话就被归档的空会话
            if (tab === "archived" && !it.title) titleChildren.push(h("span", { key: "n", style: S.badgeEmpty, title: "该会话没有标题记录（归档时就没有，例如打开后未产生对话）。若归档后标题仍显示为 id，可重启 dsh web 触发一次自动回填。" }, "无标题"));
            const children = [
            ...
            if (tab === "current") {
              children.push(h("button", { key: "a", type: "button", style: S.btn, disabled: disabled, onClick: () => act("archive", id, { force: force }), title: it.live ? "该会话已被 dsh 加载在内存中：归档后它仍会显示在会话列表里（需先重启 dsh web）；必要时可勾选「强制」硬归档" : "移出 sessions 目录（可恢复）" }, "归档"));
              ...
              children.push(h("button", { key: "c", type: "button", style: S.btnDanger, disabled: disabled, onClick: () => act("trash", id, {}), title: "删除该归档：先移进归档区内的回收站（按 trashRetentionDays 过期自动清理；界面暂不支持从中恢复，文件仍在磁盘上）" }, "删除"));
            }
      ...
      const hint = tab === "archived"
        ...
        : "「归档」= 真正移出 sessions 目录（原子移动，可随时恢复），能减少 dsh 扫描与读写；活跃会话需勾选「强制」。带「内存中」标签的会话仍被 dsh 加载着，归档后它照旧显示在会话列表里 —— 需先重启 dsh web 才能归档干净（勾选「强制」可硬归档，但不会让它从界面消失）。带「子会话」标签的是 dsh subagent 内部会话（不在侧栏显示）。";
      ...
      const fixedStyle = {
        ...
        background: "var(--dsw-alias-bg-layer-2, var(--dsw-alias-bg-base, #232323))",
        color: "var(--dsw-alias-label-primary, #e8e8e8)",
        border: "1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.3))", borderRadius: "10px",
        boxShadow: "0 10px 36px var(--dsw-alias-bg-mask-drop, rgba(0,0,0,0.45))", padding: "12px", fontSize: "12px", lineHeight: 1.5
      };
      const embeddedStyle = {
        position: "relative", width: "100%", boxSizing: "border-box",
        background: "transparent", color: "inherit", border: "none", boxShadow: "none",
        padding: "0", fontSize: "12px", lineHeight: 1.5
      };
      return h("div", { style: embedded ? embeddedStyle : fixedStyle },
        head, statLine, tabs,
        h("div", null, rows),
    ...
    function FooterButton(props) {
      ...
      const btnStyle = Object.assign({}, S.trigger, hover ? S.triggerHover : null);
      return h("div", { style: S.wrap },
        h("button", {
          type: "button",
          style: btnStyle,
          title: "会话仓库：归档 / 恢复 / 移动 / 删除会话",
          ...
          h("svg", {
            ...
            h("path", { d: "M3 8h18v12H3z" }),
            h("path", { d: "M3 8l3-4h12l3 4" }),
            h("path", { d: "M10 13h4" })
          ),
          h("span", { style: { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, "会话仓库")
        ),
    ...
    function apply(ctx) {
      // 软依赖：hub 存在 → 注册进「插件坞」（不再单独占 footer 槽位，避免挨挤）；hub 不存在 → 保留独立入口
      let registeredToHub = false;
      try {
        ...
        if (hubApi && typeof hubApi.register === "function") {
          hubApi.register({
            ...
            order: 20,
            dock: true,
            render: () => h(Boundary, null, h(Panel, { embedded: true }))
          });
