 ...
 * 侧栏底部「会话仓库」按钮 → 面板：列出当前会话（带标题/大小/工作区/轮次）与归档会话，
 * 支持 归档 / 恢复 / 移动到工作区 / 回收站 / 永久删除；面板可拖动。无构建步骤。
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      title: { fontSize: "12px", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
    ...
    class Boundary extends React.Component {
      ...
      render() {
        if (this.state.err) {
          return h("div", { style: { padding: "8px", color: "#e08585", fontSize: "11px" } },
            "会话仓库面板出错: " + String((this.state.err && this.state.err.message) || this.state.err));
        }
    ...
    function Panel(props) {
      ...
      const load = useCallback(async () => {
        try { setData(await api("/list")); } catch (e) { setMsg("加载失败: " + e.message); }
      }, []);
      useEffect(() => { load(); }, [load]);
      ...
      const startDrag = (e) => {
        ...
        const onMove = (ev) => {
          setPos({ right: Math.max(6, base.right - (ev.clientX - sx)), bottom: Math.max(6, base.bottom - (ev.clientY - sy)) });
        };
        const onUp = () => {
      ...
      const act = async (name, sessionId, extra) => {
        ...
        try {
          const body = Object.assign({ sessionId: sessionId }, extra || {});
        ...
        : list.map((it) => {
            ...
            const children = [
              h("div", { key: "i", style: S.grow },
                h("div", { style: S.title, title: label(it) }, label(it)),
                h("div", { style: S.meta },
                  fmt(it.size)
            ...
            if (tab === "current") {
              ...
              children.push(h("button", { key: "b", type: "button", style: S.btn, disabled: disabled, onClick: () => act("restore", id, {}), title: "恢复回 ~/.dsh/sessions" }, "恢复"));
              ...
              inner.push(h("div", { key: "mv", style: S.moveRow },
                ...
                wsOptions.length === 0
                  ...
                  : h("select", { style: S.select, value: target, onChange: (e) => setTarget(e.target.value) },
                      wsOptions.map((w) => h("option", { key: w.encoded, value: w.encoded },
                        shortWs(w.encoded) + (w.cwd ? "  (" + w.cwd + ")" : "") + (w.virtual ? " [新建]" : "")))),
      ...
      const hint = tab === "archived"
        ...
        : "「归档」= 真正移出 sessions 目录（原子移动，可随时恢复），能减少 dsh 扫描与读写；活跃会话需勾选「强制」。";
      const msgLine = msg ? h("div", { style: S.msg }, msg) : null;

      return h("div", {
        style: {
          position: "fixed", right: String(pos.right) + "px", bottom: String(pos.bottom) + "px",
          width: "560px", maxHeight: "76vh", overflowY: "auto", zIndex: 2147000000,
        ...
        head, statLine, tabs,
        h("div", null, rows),
        h("div", { style: { marginTop: "8px", fontSize: "11px", opacity: 0.62 } }, hint),
        msgLine
      );
    }
